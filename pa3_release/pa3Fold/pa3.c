...
void implDijkstra(Node ** graph, int source, short ncols, short nrows, int ** path, int ** accTimes) { //source is 0 node
    ...
    for(int i = 0; i < numVerts; i++) { //for some reason compweight is not getting INT Max value getting overflowed
        accTimes[source][i] = INT_MAX;
        minHeap->arr[i] = createNode(i, accTimes[source][i]);
        minHeap->pos[i] = i;
    }
    //minHeap->arr[source] = createNode(source, accTimes[source]);
    //extract source node first
    minHeap->pos[source] = source;
    accTimes[source][source] = 0;
    changeTimes(minHeap, source, accTimes[source][source]);
    ...
    while(minHeap->currSize != 0) {
        ...
        for(int i=0; i < graph[vert]->numEdges; i++) { //now iterate through all adjacent nodes
            ...
            if(inHeap(minHeap, graph[vert][i].index) && accTimes[source][vert] != INT_MAX && (graph[vert]->weight + accTimes[source][vert]) < accTimes[source][graph[vert][i].index]) {
                accTimes[source][graph[vert][i].index] = accTimes[source][vert] + graph[vert]->weight;
                //location++;
                //short temp1 = accTimes[source][graph[vert][i].index];
                // if(vert == 35) {
                //     printf("------------------------------\n");
                // }
                changeTimes(minHeap, graph[vert][i].index, accTimes[source][graph[vert][i].index]);
                path[source][graph[vert][i].index] = vert; //saving where we are coming from
...
int main(int argc, char ** argv) {
   ...
   // printf("hello\n");
    ...
    // //fprintf(stderr, "------------\n%d\n", adjMatrix[2][0].index);
    int ** path = malloc(ncols * sizeof(int*));
    int ** accTimes = malloc(ncols * sizeof(int*));
    for(int i = 0; i < ncols; i++) {
        path[i] = malloc((nums+1) * sizeof(int));
        accTimes[i] = malloc((nums+1) * sizeof(int));
    }
    ...
    for(int i = 0; i < ncols; i++) {
        //printf("hello3\n");
        //printf("hello\n");
        implDijkstra(adjMatrix, i, ncols, nrows, path, accTimes);
        //printf("hello2\n");
        if(i > 0) {
            if(accTimes[i][nums] < accTimes[i-1][nums]) {
                shortest = i;
            }
        }
    }
    ...
    //     place = path[place];
    // }
    writePath(argv[4], pathForward, place2+1, accTimes[shortest][nums], ncols);
